...
| Normalized time mapping | `open_time_ms` <- parse `candle_date_time_utc`; `close_time_ms` <- `open_time_ms + interval_ms - 1` (`mon` uses calendar-month rollover in UTC, minus 1ms) | same (`timestamp` in payload is KST candle end) | `open_time_ms` <- `timestamp`; `close_time_ms` <- `open_time_ms + interval_ms - 1` (`mon` uses calendar-month rollover in UTC, minus 1ms) | `open_time_ms` <- `timestamp` (assume UTC if undocumented); derive `close_time_ms` | `open_time_ms` <- `[0]` (assume UTC if undocumented); derive `close_time_ms` |
